...
int main(){
  ...
  norma.open("Datos/GP_GS_norm.dat",	ios::out); //check the norm how is ti used???!!  
  inist.open("Datos/Armonic_GS.dat",	ios::in); //Initial state: Gs of the initial.

  ComplexVector Phi(n), Phicte(n);
...
#pragma omp parallel for private( file1,file2,file3,trap1,Phi,C )
  for(int deep=0.; deep<=int(12./Ddeep); deep++){
...
cout<<omp_get_thread_num()<<"	"<<deep*Ddeep<<endl;
  ...
  dp << fixed << setprecision(2) << deep*Ddeep;
	...
	file1.open(dir + name + dens + "dp_" + dp.str() + ext,	ios::out);
	file2.open(dir + name + fase + "dp_" + dp.str() + ext,	ios::out);
	file3.open(dir + name + data + "dp_" + dp.str() + ext,	ios::out);
	trap1.open(dir + name + trap + "dp_" + dp.str() + ext,	ios::out);
...
//----------------------------------------------------------------------
  ...
  for(int i=0; i<n; i++){//loop
    ...
    RealVector V(n);
    V=Trap_HE(n,dx,t,xi,deep*Ddeep);
